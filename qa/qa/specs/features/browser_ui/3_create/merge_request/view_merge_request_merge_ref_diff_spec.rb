...
module QA
  RSpec.describe 'Create' do
    describe 'View merge request merge-ref diff' do
      ...
      let(:new_file) { "added_file-#{SecureRandom.hex(8)}.txt" }

      before do
        commit_to_branch(merge_request.target_branch, new_file)
        commit_to_branch(merge_request.source_branch, new_file)
      ...
      context 'when the feature flag default_merge_ref_for_diffs is enabled' do
        before do
          Runtime::Feature.enable_and_verify('default_merge_ref_for_diffs', project: merge_request.project)
        ...
        it 'views the merge-ref diff by default' do
          Page::MergeRequest::Show.perform do |mr_page|
            ...
            expect(mr_page).not_to have_file(new_file)
          end
        end
      end
      ...
      context 'when the feature flag default_merge_ref_for_diffs is disabled' do
        before do
          Runtime::Feature.disable_and_verify('default_merge_ref_for_diffs', project: merge_request.project)
        ...
        it 'views the merge-base diff by default' do
          Page::MergeRequest::Show.perform do |mr_page|
            ...
            expect(mr_page).to have_file(merge_request.file_name)
            expect(mr_page).to have_file(new_file)
          end
        end
      end
