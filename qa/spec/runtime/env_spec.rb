...
describe QA::Runtime::Env do
  ...
  describe '.require_k3d_environment!' do
    it 'raises ArgumentError if required variables are not defined' do
      stub_env('QA_K3D_HOSTNAME', nil)

      expect { described_class.require_k3d_environment! }.to raise_error(ArgumentError)
    end

    it 'doesnt raise error when required variables are defined' do
      stub_env('QA_K3D_HOSTNAME', 'something')

      expect { described_class.require_k3d_environment! }.not_to raise_error
    end
  end
