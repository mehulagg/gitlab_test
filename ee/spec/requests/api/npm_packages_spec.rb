...
describe API::NpmPackages do
  ...
  describe 'GET /api/v4/packages/npm/*package_name' do
    let(:package) { create(:npm_package, project: project, name: "@#{group.path}/#{project.name}") }
  ...
  describe 'PUT /api/v4/projects/:id/packages/npm/:package_name' do
    context 'when params are correct' do
      ...
      context 'scoped package' do
        let(:package_name) { "@#{group.path}/#{project.name}" }
        let(:params) { upload_params(package_name) }
      ...
      context 'package creation fails' do
        ...
        let(:params) { upload_params(package_name) }
        let(:service) do
          double('::Packages::CreateNpmPackageService',
            execute: { status: :error, http_status: 403, message: 'error' })
        end

        it 'returns an error' do
          expect(::Packages::CreateNpmPackageService).to receive(:new).and_return(service)
          expect { upload_package_with_token(package_name, params) }
            .not_to change { project.packages.count }
