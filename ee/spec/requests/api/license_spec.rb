...
describe API::License, api: true do
  ...
  describe 'GET /license' do
    it 'retrieves the license information if admin is logged in' do
      ...
      expect(Date.parse(json_response['expires_at'])).to eq Date.current + 11.months
  ...
  describe 'DELETE /license/:id' do
    let(:license) { create(:license, created_at: Time.current, data: build(:gitlab_license, starts_at: Date.current, expires_at: Date.current, restrictions: { add_ons: { 'GitLab_DeployBoard' => 1 }, active_user_count: 2 }).export) }
  ...
  describe 'GET /licenses' do
    ...
    let(:gl_licenses) do
      [build(:gitlab_license, starts_at: Date.current - 10, expires_at: Date.current - 1, restrictions: { add_ons: { 'GitLab_FileLocks' => 1 }, active_user_count: 10 }),
       build(:gitlab_license, starts_at: Date.current - 20, expires_at: Date.current + 1, restrictions: { add_ons: { 'GitLab_DeployBoard' => 1 }, active_user_count: 20 })]
    end
    let!(:licenses) do
