...
describe API::Internal do
  ...
  describe 'POST /internal/post_receive', :clean_gitlab_redis_shared_state, :geo do
    ...
    context 'when the push was redirected from a Geo secondary to the primary' do
      before do
        ...
        expect(git_push_http).to receive(:fetch_referrer).and_return(secondary_node.id.to_s)
      end

      context 'when the secondary has a GeoNodeStatus' do
        context 'when the GeoNodeStatus db_replication_lag_seconds is set' do
          ...
          it 'includes current Geo secondary lag in the output' do
            ...
            expect(json_response['messages'].first).to eq({
              'type' => 'alert',
              'message' => "Current replication lag to \"#{secondary_node.name}\" is less than 20 seconds."
            })
          end
        end
        ...
        context 'when the GeoNodeStatus db_replication_lag_seconds is nil' do
          ...
          it 'does not include current Geo secondary lag in the output' do
            ...
            expect(response).to have_gitlab_http_status(200)
            expect(json_response['messages']).not_to include({ 'type' => 'alert', 'message' => anything })
          end
        end
      end
    ...
    context 'when the push was not redirected from a Geo secondary to the primary' do
      before do
        expect(Gitlab::Geo::GitPushHttp).to receive(:new).with(identifier, gl_repository).and_return(git_push_http)
        expect(git_push_http).to receive(:fetch_referrer).and_return(nil)
      end
