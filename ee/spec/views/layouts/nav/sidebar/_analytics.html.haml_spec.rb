...
describe 'layouts/nav/sidebar/_analytics' do
  ...
  context 'top-level items' do
    context 'when feature flags are enabled' do
      ...
      it 'has `Code Analytics` link' do
        stub_feature_flags(Gitlab::Analytics::CODE_ANALYTICS_FEATURE_FLAG => true)

        render

        expect(rendered).to have_content('Code Analytics')
        expect(rendered).to include(analytics_code_analytics_path)
        expect(rendered).to match(/<use xlink:href=".+?icons-.+?#code">/)
      end
    end
    ...
    context 'when feature flags are disabled' do
      it 'no analytics links are rendered' do
        ...
        expect(rendered).not_to have_content('Code Analytics')
      end
    end
  end
end
