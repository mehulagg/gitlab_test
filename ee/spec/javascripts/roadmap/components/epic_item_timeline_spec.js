...
describe('EpicItemTimelineComponent', () => {
  ...
  describe('template', () => {
    ...
    it('renders component with the description in the epic bar', () => {
      vm = createComponent({
        epic: Object.assign({}, mockEpic, { startDate: mockTimeframeMonths[1] }),
        timeframeItem: mockTimeframeMonths[1],
      });

      expect(vm.$el.querySelector('.epic-bar').textContent).toContain(mockEpic.description);
    });

    it('renders component with a popover that contains the timeframe and the weight completed', () => {
      const timeframeString = 'Jul 10, 2017 – Jun 2, 2018';
      vm = createComponent({
        timeframeString,
      });

      expect(vm.$el.textContent).toContain(timeframeString);
      expect(vm.$el.textContent).toContain('3 of 5 weight completed');
    });
  });
});
