...
describe OperationsFeatureFlagsIidBackfillWorker do
  ...
  describe '#perform' do
    ...
    it 'generates iids properly for feature flags created after the worker runs' do
      project = setup
    ...
    it 'generates iids properly for feature flags created after the worker runs when flags are backfilled' do
      project = setup
      flag_a = flags.create!(project_id: project.id, active: true, name: 'test_flag')
    ...
    it 'generates iids properly for flags created after the worker runs across multiple projects' do
      project_a = setup
      project_b = setup
    ...
    context 'when the new code creates a flag post deploy but before the worker runs' do
    ...
    context 'when the new code creates a flag and then old code creates a flag post deploy but before the worker runs' do
      ...
      it 'generates an iid for a new flag after the worker runs' do
        project = setup
        flag_a = flags.create!(project_id: project.id, active: true, name: 'flag_a')
        flag_b = flags.create!(project_id: project.id, active: true, name: 'flag_b')
        flag_c = Operations::FeatureFlag.create!(project_id: project.id, active: true, name: 'flag_c')
    ...
    context 'when the new code creates and deletes a flag post deploy but before the worker runs' do
      it 'backfills flags already in the database' do
        project = setup
        flag_a = flags.create!(project_id: project.id, active: true, name: 'flag_a')
        flag_b = flags.create!(project_id: project.id, active: true, name: 'flag_b')
      ...
      it 'successfully creates a new flag after the worker runs' do
        project = setup
        flag_a = flags.create!(project_id: project.id, active: true, name: 'flag_a')
        flag_b = flags.create!(project_id: project.id, active: true, name: 'flag_b')
        flag_c = Operations::FeatureFlag.create!(project_id: project.id, active: true, name: 'flag_c')
    ...
    context 'when the new code creates and deletes a flag and old code creates a flag post deploy but before the worker runs' do
      it 'backfills flags' do
        project = setup
        flag_a = flags.create!(project_id: project.id, active: true, name: 'flag_a')
        flag_b = flags.create!(project_id: project.id, active: true, name: 'flag_b')
    ...
    context 'when the new code creates and deletes a flag and then creates another flag post deploy but before the worker runs' do
      it 'successfully generates an iid for a new flag after the worker runs' do
        project = setup
        flag_a = flags.create!(project_id: project.id, active: true, name: 'flag_a')
        flag_b = flags.create!(project_id: project.id, active: true, name: 'flag_b')
        flag_c = Operations::FeatureFlag.create!(project_id: project.id, active: true, name: 'flag_c')
