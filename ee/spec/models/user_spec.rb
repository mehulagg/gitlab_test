...
describe User do
  ...
  describe '#ab_feature_enabled?' do
    ...
    before do
      create(:user_preference, user: experiment_user, feature_filter: FEATURE_FILTER_EXPERIMENT)
      create(:user_preference, user: new_user, feature_filter: FEATURE_FILTER_UNKNOWN)
      create(:user_preference, user: new_fresh_user, feature_filter: nil)
      create(:user_preference, user: control_user, feature_filter: FEATURE_FILTER_CONTROL)
    end
    ...
    context 'when not on Gitlab.com' do
      ...
      it 'returns false' do
        users_of_different_groups.each do |user|
          expect(user.public_send(:ab_feature_enabled?, :discover_security, percentage: 100)).to eq(false)
        end
      end
    end
      ...
      context 'when not on a secondary Geo' do
        ...
        context 'for any feature except discover_security' do
          it 'raises runtime error' do
            users_of_different_groups.each do |user|
              expect do
                user.public_send(:ab_feature_enabled?, :any_other_feature, percentage: 100)
        ...
        context 'when discover_security feature flag is enabled' do
          ...
          it 'assigns to control or experiment group when feature_filter is nil' do
            expect(new_fresh_user.ab_feature_enabled?(:discover_security, percentage: 100)).not_to eq(FEATURE_FILTER_UNKNOWN)
          end

          it 'assigns to control or experiment group when feature_filter is nil' do
            expect(new_user.ab_feature_enabled?(:discover_security, percentage: 100)).not_to eq(FEATURE_FILTER_UNKNOWN)
          end
