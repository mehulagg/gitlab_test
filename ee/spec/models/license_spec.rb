...
describe License do
  ...
  describe '#exclude_guests_from_active_count?' do
    context 'when plan is ultimate' do
      let(:license) { create(:license, plan: License::ULTIMATE_PLAN) }

      it 'returns true' do
        expect(license.exclude_guests_from_active_count?).to eq(true)
      end
    end
    ...
    context 'when plan is not ultimate' do
      ...
      it 'returns false' do
        expect(license.exclude_guests_from_active_count?).to eq(false)
      end
    end
  end
