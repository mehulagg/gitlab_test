...
RSpec.describe Gitlab::Elastic::SnippetSearchResults, :elastic, :sidekiq_might_not_need_inline do
  ...
  describe '#highlight_map' do
    it 'returns the expected highlight map' do
      expect(results).to receive(:snippet_titles).and_return([{ _source: { id: 1 }, highlight: 'test <em>highlight</em>' }])
      expect(results.highlight_map('snippet_titles')).to eq({ 1 => 'test <em>highlight</em>' })
    end
  end
