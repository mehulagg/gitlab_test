...
RSpec.describe Gitlab::Ci::Parsers::Security::Common do
  describe '#parse!' do
    ...
    context 'parsing scan object' do
      it 'returns scan object' do
        expect(report.scan.type).to eq('dependency_scanning')
        expect(report.scan.status).to eq('success')
        expect(report.scan.start_time).to eq('placeholder-value')
        expect(report.scan.end_time).to eq('placeholder-value')
      end
    end
  end
end
