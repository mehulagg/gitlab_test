...
describe ConanPackagePresenter do
  ...
  describe '#package_urls' do
    subject { described_class.new(recipe, user, project, package_id).package_urls }
    ...
    context 'existing package' do
      ...
      let(:expected_result) do
        {
          "package_conaninfo.txt" => "#{Settings.build_base_gitlab_url}/api/v4/packages/conan/v1/files/#{package.conan_recipe_path}/-/0/package/#{package_id}/0/package_conaninfo.txt",
          "package_conanmanifest.txt" => "#{Settings.build_base_gitlab_url}/api/v4/packages/conan/v1/files/#{package.conan_recipe_path}/-/0/package/#{package_id}/0/package_conanmanifest.txt",
          "conan_package.tgz" => "#{Settings.build_base_gitlab_url}/api/v4/packages/conan/v1/files/#{package.conan_recipe_path}/-/0/package/#{package_id}/0/conan_package.tgz"
        }
      end
  ...
  describe '#package_snapshot' do
    subject { described_class.new(recipe, user, project, package_id).package_snapshot }
