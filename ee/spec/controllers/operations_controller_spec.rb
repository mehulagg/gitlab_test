    ...
    context 'with an authenticated developer' do
      ...
      context 'with a project in the dashboard' do
        ...
        it "returns the last deployment's user" do
          ...
          environment_json = project_json['environments'].first
          last_deployment_json = environment_json['last_deployment']
          user_json = last_deployment_json['user']
          expect(user_json['id']).to eq(user.id)
          expect(user_json['name']).to eq(user.name)
        end

        it "returns the last deployment's commit" do
          project = create(:project, :repository)
          project.add_developer(user)
          user.update!(ops_dashboard_projects: [project])
          environment = create(:environment, project: project)
          deployment = create(:deployment, project: project, environment: environment, user: user, status: :success)
          ...
          environment_json = project_json['environments'].first
          last_deployment_json = environment_json['last_deployment']
          commit_json = last_deployment_json['commit']
          expect(commit_json['short_id']).to be_present
          expect(commit_json['short_id']).to eq(commit.short_id)
          expect(commit_json['title']).to be_present
          expect(commit_json['title']).to eq(commit.title)
        end
      end
    end
  end
