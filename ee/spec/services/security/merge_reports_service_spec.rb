...
describe Security::MergeReportsService, '#execute' do
  ...
  context 'ordering reports for dependency scanning analyzers' do
    ...
    it 'sorts reports according to priority order' do
      expect(subject.scanners.values).to match_array([bundler_audit_scanner, retire_js_scaner, gemnasium_scanner])
      expect(subject.occurrences.count).to eq(2)
      expect(subject.occurrences.first.identifiers).to contain_exactly(identifier_cve)
      expect(subject.occurrences.last.identifiers).to contain_exactly(identifier_npm)
    end

    it 'puts custom analyzers to last' do
      subject = described_class.new(custom_analyzer_report, retirejs_report, bundler_audit_report).execute

      expect(subject.scanners.values).to match_array([bundler_audit_scanner, retire_js_scaner, scanner_2])
      expect(subject.occurrences.count).to eq(3)
      expect(subject.occurrences.last.identifiers).to match_array(occurrence_id_2_loc_1.identifiers)
    end
  end
end
