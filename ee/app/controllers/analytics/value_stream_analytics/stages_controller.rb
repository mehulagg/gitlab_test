  ...
  module ValueStreamAnalytics
    class StagesController < Analytics::ApplicationController
      ...
      def index
        ...
        if result.success?
          render json: value_stream_analytics_config(result.payload[:stages])
        else
      ...
      def duration_chart
        ...
        render json: Analytics::ValueStreamAnalytics::DurationChartItemEntity.represent(data_collector.duration_chart_data)
      end
      ...
      def stage
        @stage ||= Analytics::ValueStreamAnalytics::StageFinder.new(parent: @group, stage_id: params[:id]).execute
      end

      def value_stream_analytics_config(stages)
        stage_presenters = stages.map { |s| StagePresenter.new(s) }

        Analytics::ValueStreamAnalytics::ConfigurationEntity.new(stages: stage_presenters)
      end
      ...
      def render_stage_service_result(result)
        if result.success?
          ...
          render json: Analytics::ValueStreamAnalytics::StageEntity.new(stage), status: result.http_status
        else
