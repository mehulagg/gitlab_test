  ...
  if (invalid.length) {
    ...
    if (rest.length > 0) {
      invalidProjects = sprintf(
        s__('ProjectSelector|%{firstProject}, %{secondProject}, and %{rest}'),
        translationValues,
      );
    } else if (secondProject) {
      invalidProjects = sprintf(
        s__('ProjectSelector|%{firstProject} and %{secondProject}'),
        translationValues,
      );
    } else {
    ...
    createFlash(
      sprintf(s__('ProjectSelector|Unable to add %{invalidProjects}'), {
        invalidProjects,
      }),
    );
  }
...
export const receiveAddProjectsError = ({ commit, state }) => {
...
export const fetchProjects = ({ state, dispatch }) => {
  ...
  return axios
    ...
    .then(({ data }) => dispatch('receiveProjectsSuccess', data))
...
export const receiveRemoveProjectSuccess = ({ commit }) => {
...
export const fetchSearchResults = ({ state, dispatch }) => {
  ...
  if (!searchQuery || searchQuery.length < API_MINIMUM_QUERY_LENGTH) {
    return dispatch('minimumQueryMessage');
  }
