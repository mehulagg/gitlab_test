...
// import service from '../services/insights_service';
...
export default {
  ...
  computed: {
    ...mapState('insights', ['store', 'pageLoading']),
    charts() {
  ...
  methods: {
    ...mapActions('insights', ['fetchChartData', 'setInsightsStore', 'setPageLoading']),
    chartType(type) {
      switch (type) {
        case 'line':
    ...
    storePopulated() {
      const results = this.chartKeys.map(key => this.storeKeys.includes(key));
