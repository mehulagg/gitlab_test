...
import TreemapChartLegend from './treemap_chart_legend.vue';
import { DEFAULT_SPACING, DEFAULT_COLORS } from './constants';
...
export default {
  ...
  data() {
    return {
      ...
      tooltipContent: {
        title: this.tooltipContentTitle,
        value: '',
      },
      tooltipPosition: {
        left: '0',
  ...
  methods: {
    ...
    cursorInBounds(mouseEvent) {
      ...
      return (
        ...
        x < width - this.$options.spacing &&
        y > this.$options.spacing &&
        y < height - this.$options.spacing
      );
    },
    setTooltipPosition(mouseEvent) {
      this.tooltipPosition = {
        left: `${mouseEvent.zrX}px`,
        top: `${mouseEvent.zrY - this.$options.spacing}px`,
      };
    },
...
<template>
  <div>
    ...
    <treemap-chart-legend :colors="this.$options.colors" :title="legendTitle" />
  </div>
</template>
