...
export default {
  ...
  computed: {
    selectedProject() {
      const { value } = this;
      return !!value ? this.suggestions.find(project => project.id == value) : {};
    },
  },
  methods: {
  ...
  <gl-filtered-search-binary-token :title="title" :active="active" :value="value" v-on="$listeners">
    <template #view>
      ...
      <template v-else-if="value && selectedProject.id">
        <gl-avatar :size="16" :src="selectedProject.avatar_url" :entity-id="selectedProject.id" :entity-name="selectedProject.name" shape="circle" class="mr-1"/>
