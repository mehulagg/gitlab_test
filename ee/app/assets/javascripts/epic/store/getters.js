...
export default {
  isEpicOpen: state => state.state === statusType.open,

  isUserSignedIn: () => !!gon.current_user_id,

  startDateTime: state => (state.startDate ? parsePikadayDate(state.startDate) : null),

  startDateTimeFixed: state =>
    state.startDateFixed ? parsePikadayDate(state.startDateFixed) : null,

  startDateTimeFromMilestones: state =>
    state.startDateFromMilestones ? parsePikadayDate(state.startDateFromMilestones) : null,

  dueDateTime: state => (state.dueDate ? parsePikadayDate(state.dueDate) : null),

  dueDateTimeFixed: state => (state.dueDateFixed ? parsePikadayDate(state.dueDateFixed) : null),

  dueDateTimeFromMilestones: state =>
    state.dueDateFromMilestones ? parsePikadayDate(state.dueDateFromMilestones) : null,

  startDateForCollapsedSidebar: (state, getters) =>
    state.startDateIsFixed ? getters.startDateTime : getters.startDateTimeFromMilestones,

  dueDateForCollapsedSidebar: (state, getters) =>
  ...
  ancestors: state => (state.ancestors ? [...state.ancestors].reverse() : []),
};
