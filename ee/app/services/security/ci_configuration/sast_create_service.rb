  ...
  module CiConfiguration
    class SastCreateService < ::BaseService
      ...
      def track_event(attributes_for_commit)
        defaults_overwritten = attributes_for_commit[:actions].first[:default_values_overwritten].to_s
        Gitlab::Tracking.event(
          'Security::CiConfiguration::SastCreateService', attributes_for_commit[:actions].first[:action], { label: defaults_overwritten }
        )
      end
    end
  end
