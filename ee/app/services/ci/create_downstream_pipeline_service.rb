  ...
  class CreateDownstreamPipelineService < ::BaseService
    ...
    def execute(bridge)
      ...
      if target_project == project
        return bridge.drop!(:downstream_invalid_bridge_trigger)
      end

      unless can_create_cross_pipeline?
        return bridge.drop!(:downstream_insufficient_bridge_permissions)
      end
    ...
    def target_project
      strong_memoize(:target_project) do
        Project.find_by_full_path(@bridge.downstream_project_path)
      end
    end
  end
end
