...
class IssueLink < ApplicationRecord
  ...
  class << self
    ...
    def blocked_issue_ids(issue_ids)
      blocked_issues_union(issue_ids).pluck(:blocked_issue_id)
    end

    def blocking_issues_ids_for(issue)
      blocked_issues_union(issue.id).select(:blocking_issue_id)
    end
  end
  ...
  class << self
    def blocked_issues_union(issue_ids)
      from_union([
    ...
    def blocked_issues(issue_ids, link_type)
