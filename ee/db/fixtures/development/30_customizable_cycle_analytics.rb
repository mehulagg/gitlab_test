...
class Gitlab::Seeder::CustomizableCycleAnalytics
  ...
  def create_stages!
    ...
    stages_params.each do |params|
      next if Analytics::ValueStreamAnalytics::GroupStage.where(group: group).find_by(name: params[:name])

      Analytics::ValueStreamAnalytics::Stages::CreateService.new(parent: group, current_user: user, params: params).execute
    end
  end
