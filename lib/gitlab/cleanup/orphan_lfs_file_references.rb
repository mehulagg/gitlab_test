...
module Gitlab
  module Cleanup
    class OrphanLfsFileReferences
      ...
      # This parameter is required by Gitaly GetAllLfsPointers endpoint
      # but it will ultimately be ignored. The bug is known https://gitlab.com/gitlab-org/gitaly/issues/2247
      # Once it's fixed we should remove it.
      REVISION = 'HEAD'
      ...
      def lfs_oids_from_repository
        project.repository.gitaly_blob_client.get_all_lfs_pointers(REVISION).map(&:lfs_oid)
      end
