    ...
    class ServerMetrics < SidekiqMiddleware::Metrics
      ...
      def call(worker, job, queue)
        ...
        begin
          ...
          record_redis_metrics(job, labels)
        end
      end
      ...
      def init_metrics
        {
          ...
          sidekiq_redis_query_duration_seconds: ::Gitlab::Metrics.histogram(:sidekiq_redis_query_duration_seconds, 'Duration in seconds that a Sidekiq job spent querying a Redis server', {}, Gitlab::Instrumentation::Redis::QUERY_TIME_BUCKETS),
          sidekiq_jobs_failed_total:            ::Gitlab::Metrics.counter(:sidekiq_jobs_failed_total, 'Sidekiq jobs failed'),
          sidekiq_jobs_retried_total:           ::Gitlab::Metrics.counter(:sidekiq_jobs_retried_total, 'Sidekiq jobs retried'),
          sidekiq_redis_calls_total:            ::Gitlab::Metrics.counter(:sidekiq_redis_calls_total, 'Redis calls during a Sidekiq job execution'),
      ...
      def record_redis_metrics(job, labels)
        redis_calls = get_redis_calls(job)

        return unless redis_calls > 0

        @metrics[:sidekiq_redis_calls_total].increment(labels, redis_calls)
        @metrics[:sidekiq_redis_query_duration_seconds].observe(labels, get_redis_time(job))
      end
