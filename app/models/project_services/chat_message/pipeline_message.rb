...
module ChatMessage
  class PipelineMessage < BaseMessage
    ...
    def failed_jobs_links
      ...
      # Instead of showing "and 1 more", just show the final job
      # Start showing "and x more" when (limit - 1) is exceeded by at least 2
      if failed_jobs.length > MAX_VISIBLE_JOBS
        truncated_failed_jobs = failed_jobs.take(MAX_VISIBLE_JOBS - 1)
        more_text = s_("ChatMessage|and [%{count} more](%{pipeline_jobs_url})") %
          { count: failed_jobs.length - (MAX_VISIBLE_JOBS - 1), pipeline_jobs_url: pipeline_jobs_url }
      end
