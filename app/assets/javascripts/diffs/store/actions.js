...
export default {
  ...
  setHighlightedRow: ({ commit }, lineCode) => {
  ...
  removeDiscussionsFromDiff: ({ commit }, removeDiscussion) => {
    const { file_hash, line_code, id } = removeDiscussion;
    commit(types.REMOVE_LINE_DISCUSSIONS_FOR_FILE, {
      fileHash: file_hash,
      lineCode: line_code,
      id,
    });
  ...
  renderFileForDiscussionId: ({ commit, rootState, state }, discussionId) => {
  ...
  startRenderDiffsQueue: ({ state, commit }) => {
    const checkItem = () =>
      new Promise(resolve => {
        const nextFile = state.diffFiles.find(
          file =>
            !file.renderIt &&
            (!file.viewer.collapsed || !file.viewer.name === diffViewerModes.text),
        );
  ...
  setRenderIt: ({ commit }, file) => commit(types.RENDER_FILE, file),

  setInlineDiffViewType: ({ commit }) => {
  ...
  setParallelDiffViewType: ({ commit }) => {
  ...
  scrollToLineIfNeededInline: (_, line) => {
  ...
  scrollToLineIfNeededParallel: (_, line) => {
    const hash = getLocationHash();
   ...
   * @param {Object} diff
   */
  toggleFileDiscussions: ({ getters, dispatch }, diff) => {
  ...
  toggleShowTreeList: ({ commit, state }, saving = true) => {
  ...
  openDiffFileCommentForm: ({ commit, getters }, formData) => {
  ...
  toggleFileFinder: ({ commit }, visible) => {
    commit(types.TOGGLE_FILE_FINDER_VISIBLE, visible);
  },

  cacheTreeListWidth: (_, size) => {
    localStorage.setItem(TREE_LIST_WIDTH_STORAGE_KEY, size);
  },

  requestFullDiff: ({ commit }, filePath) => commit(types.REQUEST_FULL_DIFF, filePath),
  receiveFullDiffSucess: ({ commit }, { filePath, data }) =>
    commit(types.RECEIVE_FULL_DIFF_SUCCESS, { filePath, data }),
  receiveFullDiffError: ({ commit }, filePath) => {
    commit(types.RECEIVE_FULL_DIFF_ERROR, filePath);
    createFlash(s__('MergeRequest|Error loading full diff. Please try again.'));
  },

  fetchFullDiff: ({ dispatch }, file) =>
    axios
  ...
  toggleFullDiff: ({ dispatch, getters, state }, filePath) => {
  ...
  setFileCollapsed: ({ commit }, { filePath, collapsed }) =>
