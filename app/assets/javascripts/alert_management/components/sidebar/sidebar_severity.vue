...
<template>
  <div class="block alert-status">
    <div ref="status" class="sidebar-collapsed-icon" @click="onClickCollapsedIcon">
      ...
      <p v-else class="collapse-truncated-title px-1">{{ $options.severityLabels[severity] }}</p>
    </div>
    ...
    <div class="hide-collapsed">
      ...
      >
        <gl-dropdown
          ref="dropdown"
          :text=" $options.severityLabels[severity]"
          class="w-100"
            ...
            >
              <span class="d-flex">
                <gl-icon
                  ...
                  :class="{ invisible: label.toUpperCase() !== severity }"
      ...
      <p
        ...
        class="value m-0"
        :class="{ 'no-value': ! $options.severityLabels[severity] }"
      >
        <span v-if=" $options.severityLabels[severity]" class="text-plain bold">{{
           $options.severityLabels[severity]
        }}</span>
        <span v-else>
