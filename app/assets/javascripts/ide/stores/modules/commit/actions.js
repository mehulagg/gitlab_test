...
export default {
  updateCommitMessage: ({ commit }, message) => {
  ...
  updateCommitAction: ({ commit, rootGetters }, commitAction) => {
  ...
  toggleShouldCreateMR: ({ commit }) => {
    commit(types.TOGGLE_SHOULD_CREATE_MR);
  },

  updateBranchName: ({ commit }, branchName) => {
    commit(types.UPDATE_NEW_BRANCH_NAME, branchName);
  },

  setLastCommitMessage: ({ commit, rootGetters }, data) => {
  ...
  updateFilesAfterCommit: ({ commit, dispatch, rootState, rootGetters }, { data }) => {
  ...
  commitChanges: ({ commit, state, getters, dispatch, rootState, rootGetters }) => {
