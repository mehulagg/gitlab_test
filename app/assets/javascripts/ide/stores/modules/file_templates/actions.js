...
export default {
  requestTemplateTypes: ({ commit }) => commit(types.REQUEST_TEMPLATE_TYPES),
  receiveTemplateTypesError: ({ commit, dispatch }) => {
  ...
  receiveTemplateTypesSuccess: ({ commit }, templates) =>
    commit(types.RECEIVE_TEMPLATE_TYPES_SUCCESS, templates),

  fetchTemplateTypes: ({ dispatch, state, rootState }, page = 1) => {
    if (!Object.keys(state.selectedTemplateType).length) return Promise.reject();

    dispatch('requestTemplateTypes');

    return Api.projectTemplates(rootState.currentProjectId, state.selectedTemplateType.key, {
      page,
    })
      .then(({ data, headers }) => {
  ...
  setSelectedTemplateType: ({ commit, dispatch, rootGetters }, type) => {
  ...
  receiveTemplateError: ({ dispatch }, template) => {
    dispatch(
      'setErrorMessage',
      {
  ...
  fetchTemplate: ({ dispatch, state, rootState }, template) => {
    if (template.content) {
  ...
  setFileTemplate: ({ dispatch, commit, rootGetters }, template) => {
    dispatch(
      'changeFileContent',
    ...
    eventHub.$emit(
      `editor.update.model.new.content.${rootGetters.activeFile.key}`,
      template.content,
    );
  },

  undoFileTemplate: ({ dispatch, commit, rootGetters }) => {
