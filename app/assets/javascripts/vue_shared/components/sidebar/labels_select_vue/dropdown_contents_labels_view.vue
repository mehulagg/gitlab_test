...
export default {
  ...
  mounted() {
    this.fetchLabels();
    this.$parent.$el.addEventListener('keydown', this.handleKeyDown);
  },
  beforeDestroy() {
    this.$parent.$el.removeEventListener('keydown', this.handleKeyDown);
  },
  methods: {
    ...mapActions([
...
<template>
  <div class="labels-select-contents-list">
    <gl-loading-icon
