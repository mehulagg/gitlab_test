...
export const requestPackagesList = ({ dispatch, state }, params = {}) => {
  ...
  return Api[apiMethod](state.config.resourceId, {
    ...
    .catch(() => {
      deprecatedCreateFlash(FETCH_PACKAGES_LIST_ERROR_MESSAGE);
    })
    .finally(() => {
...
export const requestDeletePackage = ({ dispatch, state }, { _links }) => {
  if (!_links || !_links.delete_api_path) {
    deprecatedCreateFlash(DELETE_PACKAGE_ERROR_MESSAGE);
  ...
  return axios
    ...
    .then(() => {
      ...
      deprecatedCreateFlash(DELETE_PACKAGE_SUCCESS_MESSAGE, 'success');
    })
    .catch(() => {
      dispatch('setLoading', false);
      deprecatedCreateFlash(DELETE_PACKAGE_ERROR_MESSAGE);
