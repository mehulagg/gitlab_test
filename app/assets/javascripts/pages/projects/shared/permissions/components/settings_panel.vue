...
export default {
  ...
  data() {
    const defaults = {
      ...
      showAutoUpdateForksWarning: false,
    };
  ...
  computed: {
    ...
    forkingLevelOptions() {
      return [
        [forkingAccessLevelOptions.ALLOW_FORKS, forkingAccessLevelDescriptions.ALLOW_FORKS],
        [
          forkingAccessLevelOptions.PRIVATE_FORKS_ONLY,
          forkingAccessLevelDescriptions.PRIVATE_FORKS_ONLY,
        ],
      ];
    },
    ...
    showForkingAccessLevel() {
      return gon.features && gon.features.showForkingAccessLevel && this.isPrivateProject;
    },
  },
  ...
  watch: {
    visibilityLevel(value, oldValue) {
      ...
      // show warning when switching from private/public to internal
      this.showAutoUpdateForksWarning =
        (oldValue === visibilityOptions.PRIVATE || oldValue === visibilityOptions.PUBLIC) &&
  ...
  <div>
    <div class="project-visibility-setting">
      <project-setting-row :help-path="visibilityHelpPath" :label="__('Project visibility')">
        ...
        <div v-if="showAutoUpdateForksWarning" class="bs-callout bs-callout-warning mt-2">
          {{ __('Any existing Public forks will be set to Internal.') }}
        </div>
