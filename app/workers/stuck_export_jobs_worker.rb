...
class StuckExportJobsWorker
  ...
  def mark_stuck_jobs_as_failed!
    ...
    Rails.logger.info("Marked stuck export jobs as failed. JIDs: #{completed_jobs.map(&:jid).join(', ')}") # rubocop:disable Gitlab/RailsLogger
  ...
  def enqueued_exports
    ProjectExportJob.with_status([:started])
  end
end
