...
module SearchHelper
  ...
  def search_entries_info_template(collection)
    if collection.total_pages > 1
      term = "<span>&nbsp;<code>%{term}</code>&nbsp;</span>"
      s_("SearchResults|Showing %{from} - %{to} of %{count} %{scope} for#{term}").html_safe
    else
      term = "<span>&nbsp;<code>%{term}</code>&nbsp;</span>"
      s_("SearchResults|Showing %{count} %{scope} for#{term}").html_safe
    end
  end
