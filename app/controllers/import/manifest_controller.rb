...
class Import::ManifestController < Import::BaseController
  ...
  def upload
    ...
    if manifest.valid?
      manifest_import_status.save(manifest.projects, group.id)
  ...
  # rubocop: disable CodeReuse/ActiveRecord
  def group
    @group ||= Group.find_by(id: manifest_import_status.group_id)
  end
  # rubocop: enable CodeReuse/ActiveRecord

  def manifest_import_status
    @manifest_import_status ||= Gitlab::ManifestImport::Status.new(current_user, fallback: session)
  end

  def repositories
    @repositories ||= manifest_import_status.repositories
  end
