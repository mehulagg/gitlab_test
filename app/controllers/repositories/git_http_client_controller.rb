...
class Repositories::GitHttpClientController < Repositories::ApplicationController
  ...
  def parse_repo_path
    @repositorable, @project, @repo_type, @redirected_path = Gitlab::RepoPath.parse("#{params[:namespace_id]}/#{params[:repository_id]}")
  end
  ...
  def repository
    strong_memoize(:repository) do
      repo_type.repository_for(@repositorable)
    end
  end
  ...
  def http_download_allowed?
    ...
    download_request? &&
    (project && Guest.can?(:download_code, project) || @repositorable.is_a?(PersonalSnippet))
  end
end
