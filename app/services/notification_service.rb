...
class NotificationService
  ...
  def reassigned_issue(issue, current_user, previous_assignees = [])
    recipients = NotificationRecipients::Service.build_recipients(
      issue,
      current_user,
      action: "reassign",
  ...
  def push_to_merge_request(merge_request, current_user, new_commits: [], existing_commits: [])
    ...
    recipients = NotificationRecipients::Service.build_recipients(merge_request, current_user, action: "push_to")
  ...
  def send_new_note_notifications(note)
    ...
    recipients = NotificationRecipients::Service.build_new_note_recipients(note)
    recipients.each do |recipient|
  ...
  def send_new_release_notifications(release)
    recipients = NotificationRecipients::Service.build_new_release_recipients(release)
  ...
  def issue_moved(issue, new_issue, current_user)
    recipients = NotificationRecipients::Service.build_recipients(issue, current_user, action: 'moved')
  ...
  def new_resource_email(target, method)
    recipients = NotificationRecipients::Service.build_recipients(target, target.author, action: "new")
  ...
  def new_mentions_in_resource_email(target, new_mentioned_users, current_user, method)
    recipients = NotificationRecipients::Service.build_recipients(target, current_user, action: "new")
  ...
  def reopen_resource_email(target, current_user, method, status)
    recipients = NotificationRecipients::Service.build_recipients(target, current_user, action: "reopen")
  ...
  def merge_request_unmergeable_email(merge_request)
    recipients = NotificationRecipients::Service.build_merge_request_unmergeable_recipients(merge_request)
  ...
  def project_maintainers_recipients(target, action:)
    NotificationRecipients::Service.build_project_maintainers_recipients(target, action: action)
  end

  def notifiable?(*args)
    NotificationRecipients::Service.notifiable?(*args)
  end

  def notifiable_users(*args)
    NotificationRecipients::Service.notifiable_users(*args)
  end
