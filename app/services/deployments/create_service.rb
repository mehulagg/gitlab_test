...
module Deployments
  class CreateService
    ...
    def create_deployment_for_deployable!(deployable)
      # We use this approach here so that the deployment is immediately
      # available on the `deployable` object, without the need for a reload.
      deployable.create_deployment!(deployment_attributes)
    end
