...
class MergeRequestWidgetEntity < Grape::Entity
  ...
  expose :user_callouts_path, if: -> (*) { Gitlab::Experimentation.experiment_enabled?(:suggest_pipeline) } do |_merge_request|
    user_callouts_path
  end

  expose :suggest_pipeline_feature_id, if: -> (*) { Gitlab::Experimentation.experiment_enabled?(:suggest_pipeline) } do |_merge_request|
    SUGGEST_PIPELINE
  end

  expose :is_dismissed_suggest_pipeline, if: -> (*) { Gitlab::Experimentation.experiment_enabled?(:suggest_pipeline) } do |_merge_request|
