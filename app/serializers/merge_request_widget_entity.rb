...
class MergeRequestWidgetEntity < Grape::Entity
  ...
  expose :blob_path do
    expose :head_path, if: -> (mr, _) { mr.head_pipeline.commit } do |merge_request|
      project_blob_path(merge_request.project, merge_request.head_pipeline.commit)
    end

    expose :base_path, if: -> (mr, _) { mr.base_pipeline.commit } do |merge_request|
      project_blob_path(merge_request.project, merge_request.base_pipeline.commit)
    end
  end
