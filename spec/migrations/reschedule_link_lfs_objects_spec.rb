...
require 'spec_helper'
require Rails.root.join('db', 'post_migrate', '20200306134708_reschedule_link_lfs_objects.rb')
  ...
  context 'when there are forks to be backfilled' do
    ...
    it 'schedules background migration to link LFS objects' do
      Sidekiq::Testing.fake! do
        ...
        expect(described_class::MIGRATION)
          .to be_scheduled_delayed_migration(4.minutes, other_project.id, partially_linked_project.id)
      end
    end
  end
