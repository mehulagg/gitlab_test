  ...
  describe '/projects/:id/issues/:issue_iid/move' do
    ...
    context 'when source and target projects are the same' do
      it 'returns 400 when trying to move an issue' do
        ...
        expect(response).to have_gitlab_http_status(400)
        expect(json_response['message']).to eq(_('MoveIssue|Cannot move issue to project it originates from!'))
      end
    end
    ...
    context 'when the user does not have the permission to move issues' do
      it 'returns 400 when trying to move an issue' do
        ...
        expect(response).to have_gitlab_http_status(400)
        expect(json_response['message']).to eq(_('MoveIssue|Cannot move issue due to insufficient permissions!'))
      end
    end
