...
describe RegistrationsController do
  ...
  describe '#destroy' do
    ...
    def expect_password_failure
      expect_failure(_('Profiles|Invalid password'))
    end

    def expect_username_failure
      expect_failure(_('Profiles|Invalid username'))
    end

    def expect_success
      expect(flash[:notice]).to eq _('Profiles|Account scheduled for removal.')
      expect(response.status).to eq(303)
      expect(response).to redirect_to new_user_session_path
    end
