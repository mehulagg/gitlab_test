...
RSpec.describe Import::ManifestController, :clean_gitlab_redis_shared_state do
  ...
  describe 'POST upload' do
    context 'with a valid manifest' do
      it 'saves the manifest and redirects to the status page', :aggregate_failures do
        ...
        status = Gitlab::ManifestImport::Status.new(user)

        expect(status.group_id).to eq(group.id)
        expect(status.repositories.size).to eq(660)
        expect(status.repositories.first).to include(name: 'platform/build', path: 'build/make')
  ...
  describe 'GET status' do
    ...
    context 'when the data is stored via Gitlab::ManifestImport::Status' do
      before do
        Gitlab::ManifestImport::Status.new(user).save(repos, group.id)
      end
