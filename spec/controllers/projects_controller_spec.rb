    ...
    shared_examples_for 'updating a project' do
      ...
      context 'when project has container repositories with tags' do
        ...
        it 'does not allow to rename the project' do
          ...
          expect(controller).to set_flash.now[:alert].to(_('UpdateProject|Cannot rename project because it contains container registry tags!'))
          expect(response).to have_gitlab_http_status(200)
        end
      end
  ...
  describe '#transfer' do
    ...
    context 'when new namespace is empty' do
      it 'project namespace is not changed' do
        ...
        expect(response).to have_gitlab_http_status(200)
        expect(flash[:alert]).to eq _('TransferProject|Please select a new namespace for your project.')
      end
    end
  end
