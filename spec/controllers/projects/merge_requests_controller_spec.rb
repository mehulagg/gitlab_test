  ...
  describe 'GET accessibility_reports' do
    ...
    context 'when pipeline has jobs with accessibility reports' do
      ...
      context 'when user created corrupted accessibility reports' do
        let(:accessibility_comparison) { { status: :error, status_reason: 'Failed to parse accessibility reports' } }
        ...
        it 'returns 400 HTTP status' do
          ...
          expect(response).to have_gitlab_http_status(:bad_request)
          expect(json_response).to eq({ 'status_reason' => 'Failed to parse accessibility reports' })
        end
      end
    end
  end
