...
describe JobEntity do
  ...
  context 'when job failed' do
    ...
    it 'states that it failed' do
      expect(subject[:status][:label]).to eq(_('CiStatusLabel|failed'))
    end

    it 'indicates the failure reason on tooltip' do
      expect(subject[:status][:tooltip]).to eq("#{_('CiStatusLabel|failed')} - (API failure)")
    end
  ...
  context 'when job is allowed to fail' do
    ...
    it 'indicates the failure reason on tooltip' do
      expect(subject[:status][:tooltip]).to eq("#{_('CiStatusLabel|failed')} - (API failure) (allowed to fail)")
    end
