      ...
      context 'when gmake or make available' do
        ...
        context 'gmake is available' do
          ...
          it 'calls gmake in the gitaly directory' do
            allow(main_object).to receive(:make_cmd).and_return('/usr/bin/gmake')
            expect(main_object).to receive(:run_command!).with(command_preamble + %w[/usr/bin/gmake]).and_return(true)
        ...
        context 'gmake is not available' do
          before do
            expect(main_object).to receive(:checkout_or_clone_version)
            allow(main_object).to receive(:make_cmd).and_return('/usr/bin/make')
          end

          it 'calls make in the gitaly directory' do
            expect(main_object).to receive(:run_command!).with(command_preamble + %w[/usr/bin/make]).and_return(true)
