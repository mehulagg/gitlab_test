...
describe SourcegraphHelper do
  ...
  context '#sourcegraph_experimental_message' do
    ...
    context 'when limited by feature' do
      ...
      it { is_expected.to eq "This feature is experimental and currently limited to only certain projects." }
    end

    context 'when limited by public only' do
      let(:public_only) { true }

      it { is_expected.to eq "This feature is experimental and limited to only public projects." }
    end
  end
end
