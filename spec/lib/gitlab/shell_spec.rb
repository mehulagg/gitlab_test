  ...
  describe 'projects commands' do
    ...
    describe '#remove_repository' do
      ...
      it 'returns true when the command succeeds' do
        expect(exists?(project.repository_storage, disk_path)).to be(true)

        expect(gitlab_shell.remove_repository(project.repository_storage, project.disk_path)).to be(true)

        expect(exists?(project.repository_storage, disk_path)).to be(false)
      end
      ...
      it 'keeps the namespace directory' do
        ...
        expect(exists?(project.repository_storage, disk_path)).to be(false)
        expect(exists?(project.repository_storage, project.disk_path.gsub(project.name, ''))).to be(true)
      end
    end
    ...
    describe '#mv_repository' do
      ...
      it 'returns true when the command succeeds' do
        ...
        expect(exists?(project2.repository_storage, "#{old_path}.git")).to be(true)
        expect(exists?(project2.repository_storage, "#{new_path}.git")).to be(false)

        expect(gitlab_shell.mv_repository(project2.repository_storage, old_path, new_path)).to be_truthy

        expect(exists?(project2.repository_storage, "#{old_path}.git")).to be(false)
        expect(exists?(project2.repository_storage, "#{new_path}.git")).to be(true)
      end

      it 'returns false when the command fails' do
        expect(gitlab_shell.mv_repository(project2.repository_storage, project2.disk_path, '')).to be_falsy
        expect(exists?(project2.repository_storage, "#{project2.disk_path}.git")).to be(true)
      end
    end
  ...
  describe 'namespace actions' do
    ...
    describe '#add_namespace' do
      it 'creates a namespace' do
        ...
        expect(exists?(storage, "mepmep")).to be(true)
      end
    end
    ...
    describe '#remove' do
      it 'removes the namespace' do
        ...
        expect(exists?(storage, "mepmep")).to be(false)
      end
    end
    ...
    describe '#mv_namespace' do
      it 'renames the namespace' do
        ...
        expect(exists?(storage, "mepmep")).to be(false)
        expect(exists?(storage, "2mep")).to be(true)
      end
    end
  end
end
