...
RSpec.describe Gitlab::Kubernetes::NetworkPolicyCommon do
  ...
  describe '#generate' do
    ...
    context 'with spec method implemented' do
      before do
        allow(policy).to receive(:spec).and_return({ selector: selector })
      end
      ...
      context 'with api_version class method implement' do
        ...
        before do
          allow(Gitlab::Kubernetes::NetworkPolicyCommon).to receive(:api_version).and_return(api_version)
        end
  ...
  define '#standard?' do
    ...
    context 'with NetworkPolicy class' do
      before do
        allow(policy).to receive(:is_a?).and_return(true)
      end
