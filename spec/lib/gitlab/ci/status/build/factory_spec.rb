...
describe Gitlab::Ci::Status::Build::Factory do
  ...
  context 'when build is successful' do
    ...
    it 'fabricates status with correct details' do
      expect(status.text).to eq _('CiStatusText|passed')
      expect(status.icon).to eq 'status_success'
      expect(status.favicon).to eq 'favicon_status_success'
      expect(status.label).to eq _('CiStatusLabel|passed')
  ...
  context 'when build is failed' do
    context 'when build is not allowed to fail' do
      ...
      it 'fabricates status with correct details' do
        expect(status.text).to eq _('CiStatusText|failed')
        expect(status.icon).to eq 'status_failed'
        expect(status.favicon).to eq 'favicon_status_failed'
        expect(status.label).to eq _('CiStatusLabel|failed')
        expect(status.status_tooltip).to eq "#{_('CiStatusText|failed')} - (unknown failure)"
        expect(status).to have_details
        expect(status).to have_action
      end
    end
  ...
  context 'when build is a canceled' do
    ...
    it 'fabricates status with correct details' do
      expect(status.text).to eq _('CiStatusText|canceled')
      ...
      expect(status.label).to eq _('CiStatusLabel|canceled')
      expect(status).to have_details
      expect(status).to have_action
    end
  end
  ...
  context 'when build is pending' do
    ...
    it 'fabricates status with correct details' do
      expect(status.text).to eq _('CiStatusText|pending')
      ...
      expect(status.label).to eq _('CiStatusLabel|pending')
      expect(status).to have_details
      expect(status).to have_action
    end
  end
  ...
  context 'when build is skipped' do
    ...
    it 'fabricates status with correct details' do
      expect(status.text).to eq _('CiStatusText|skipped')
      expect(status.icon).to eq 'status_skipped'
      expect(status.favicon).to eq 'favicon_status_skipped'
      expect(status.illustration).to include(:image, :size, :title)
      expect(status.label).to eq _('CiStatusLabel|skipped')
      expect(status).to have_details
  ...
  context 'when build is a manual action' do
    context 'when build is a play action' do
      ...
      it 'fabricates status with correct details' do
        expect(status.text).to eq _('CiStatusText|manual')
  ...
  context 'when build is a delayed action' do
    ...
    it 'fabricates status with correct details' do
      expect(status.text).to eq _('CiStatusText|delayed')
