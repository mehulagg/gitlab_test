  ...
  context "with worker attribution" do
    ...
    shared_examples "a metrics middleware" do
      context "with mocked prometheus" do
        ...
        let(:redis_calls_total_metric) { double('redis calls total metric') }
        ...
        before do
          ...
          allow(Gitlab::Metrics).to receive(:histogram).with(:sidekiq_redis_query_duration_seconds, anything, anything, anything).and_return(redis_seconds_metric)
          allow(Gitlab::Metrics).to receive(:counter).with(:sidekiq_jobs_failed_total, anything).and_return(failed_total_metric)
          allow(Gitlab::Metrics).to receive(:counter).with(:sidekiq_jobs_retried_total, anything).and_return(retried_total_metric)
          allow(Gitlab::Metrics).to receive(:counter).with(:sidekiq_redis_calls_total, anything).and_return(redis_calls_total_metric)
        ...
        describe '#call' do
          ...
          before do
            ...
            allow(redis_calls_total_metric).to receive(:increment)
          ...
          it 'sets queue specific metrics' do
            ...
            expect(redis_calls_total_metric).to receive(:increment).with(labels_with_job_status, redis_calls)

            subject.call(worker, job, :test) { nil }
          end

          context 'when there are no redis calls' do
            let(:redis_calls) { 0 }
            let(:redis_duration) { 0 }

            it 'does not set redis metrics' do
              expect(redis_seconds_metric).not_to receive(:observe)
              expect(redis_calls_total_metric).not_to receive(:increment)
            end
          end
