...
describe ::Gitlab::RepoPath do
  ...
  describe '.parse' do
    context 'a repository storage path' do
      ...
      it 'parses a personal snippet repository path' do
        expect(described_class.parse(personal_snippet.repository.full_path)).to eq([personal_snippet, nil, Gitlab::GlRepository::SNIPPET, nil])
      end

      it 'parses a project snippet repository path' do
        expect(described_class.parse(project_snippet.repository.full_path)).to eq([project_snippet, project, Gitlab::GlRepository::SNIPPET, nil])
      end
    end
    ...
    context 'a relative path' do
      ...
      context 'of a redirected project' do
        ...
        it 'parses a redirected project snippet repository path' do
          expect(described_class.parse(redirect.path + "/snippets/#{project_snippet.id}.git")).to eq([project_snippet, project, Gitlab::GlRepository::SNIPPET, 'foo/bar/snippets/2'])
        end
      end
    end
  ...
  describe '.find_snippet' do
    it 'extracts path and id from personal snippet route' do
      expect(described_class.find_snippet(personal_snippet.repository.full_path)).to eq([personal_snippet, false])
    end

    it 'extracts path and id from project snippet route' do
      expect(described_class.find_snippet(project_snippet.repository.full_path)).to eq([project_snippet, false])
    end

    it 'returns nil for invalid snippet types' do
