...
describe Gitlab::BackgroundMigration::LinkLfsObjects, :migration, schema: 2020_02_10_062432 do
  ...
  before do
    ...
    [project, another_project, partially_linked_project, fully_linked_project].each do |p|
    ...
    [source_project, another_source_project, fully_linked_project].each do |p|
  ...
  it 'creates LfsObjectsProject records for forks within the specified range of project IDs' do
    ...
    expect { subject.perform(project.id, partially_linked_project.id) }.to change { lfs_objects_projects.count }.by(7)
    ...
    expect { subject.perform(project.id, partially_linked_project.id) }.not_to change { lfs_objects_projects.count }
  end

  context 'when it is not necessary to create LfsObjectProject records' do
    it 'does not create LfsObjectProject records' do
      expect { subject.perform(fully_linked_project.id, fully_linked_project.id) }
