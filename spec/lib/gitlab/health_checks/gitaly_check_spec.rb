...
describe Gitlab::HealthChecks::GitalyCheck do
  ...
  describe '#readiness' do
    ...
    context 'Gitaly server is up' do
      ...
      it { is_expected.to eq([result_class.new('gitaly_check::default', true, nil, shard: 'default')]) }
    end

    context 'Gitaly server is down' do
      let(:gitaly_check) { double(check: { success: false, message: 'Connection refused' }) }

      it { is_expected.to eq([result_class.new('gitaly_check::default', false, 'Connection refused', shard: 'default')]) }
    end
  end
