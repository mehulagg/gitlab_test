...
shared_examples 'base stage' do
  ...
  before do
    ...
    allow_next_instance_of(Gitlab::ValueStreamAnalytics::BaseEventFetcher) do |instance|
...
shared_examples 'Gitlab::Analytics::ValueStreamAnalytics::DataCollector backend examples' do
  let(:stage_params) { Gitlab::Analytics::ValueStreamAnalytics::DefaultStages.send("params_for_#{stage_name}_stage").merge(project: project) }
  let(:stage) { Analytics::ValueStreamAnalytics::ProjectStage.new(stage_params) }
  let(:data_collector) { Gitlab::Analytics::ValueStreamAnalytics::DataCollector.new(stage: stage, params: { from: stage_options[:from], current_user: project.creator }) }
