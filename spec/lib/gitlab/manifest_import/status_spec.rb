...
RSpec.describe Gitlab::ManifestImport::Status, :clean_gitlab_redis_shared_state do
  ...
  describe '#save' do
    it 'stores data in Redis with an expiry of EXPIRY_TIME' do
      ...
      Gitlab::Redis::SharedState.with do |redis|
        expect(redis.ttl(status.key_for('repositories'))).to be_within(5).of(described_class::EXPIRY_TIME)
        expect(redis.ttl(status.key_for('group_id'))).to be_within(5).of(described_class::EXPIRY_TIME)
      end
    end
  end
