  ...
  describe('validationErrors', () => {
    ...
    describe('when the form is invalid', () => {
      ...
      it('returns a validation error if the URL is in the wrong format', () => {
        ...
        expect(actualErrors).toEqual(expect.objectContaining(expectedErrors));
      });
      ...
      it('returns a validation error if the URL missing (and the title is populated)', () => {
        const expectedErrors = {
          assets: {
            links: expect.objectContaining({
              // Using expect.objectContaining here because these
              // URLs are also considered bad format and duplicate
              6: expect.objectContaining({ isUrlEmpty: true }),
              5: expect.objectContaining({ isUrlEmpty: true }),
            }),
          },
        };
