...
describe('Repository table row component', () => {
  ...
  it.each`
    ...
    ${'file'}   | ${false}
    ${'commit'} | ${false}
  `('renders router link when $type is tree', ({ type, routerLink }) => {
    factory({
      id: '1',
      sha: '123',
      path: 'test',
    ...
    return vm.vm.$nextTick().then(() => {
      if (routerLink) {
        expect(vm.find({ ref: 'link' }).props('to')).toEqual({ path: '/-/tree/master/test' });
      } else {
        expect(vm.find({ ref: 'link' }).attributes('href')).toBe('https://test.com');
      }
    });
  });
  ...
  it('pushes new route for directory with hash', () => {
    ...
    return vm.vm.$nextTick().then(() => {
      expect(vm.find({ ref: 'link' }).props('to')).toEqual({ path: '/-/tree/master/test%23' });
    });
  });
