...
import { skippedTestCases, erroredTestCases } from './mock_data';
...
describe('Test reports suite table', () => {
  ...
  testSuite.test_cases = [...testSuite.test_cases, ...skippedTestCases, ...erroredTestCases];
  ...
  describe('when a test suite is supplied', () => {
    ...
    it('renders the failed tests first, skipped tests next, then successful tests', () => {
      const expectedCaseOrder = [
        ...testCases.filter(x => x.status === TestStatus.ERRORED),
    ...
    it('renders the correct icon for each status', () => {
      const erroredTest = testCases.findIndex(x => x.status === TestStatus.ERRORED);
      ...
      const erroredRow = findCaseRowAtIndex(erroredTest);
      ...
      expect(findIconForRow(erroredRow, TestStatus.ERRORED).exists()).toBe(true);
