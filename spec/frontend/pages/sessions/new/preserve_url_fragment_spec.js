  ...
  it('does not add an empty query parameter to OmniAuth login buttons', () => {
    ...
    expect(
      $('#oauth-login-cas3')
        .parent('form')
        .attr('action'),
    ).toBe('http://test.host/users/auth/cas3');

    expect(
      $('.omniauth-container #oauth-login-auth0')
        .parent('form')
        .attr('action'),
    ).toBe('http://test.host/users/auth/auth0');
  });
    ...
    it('when "remember_me" is not present', () => {
      ...
      expect(
        $('#oauth-login-cas3')
          .parent('form')
          .attr('action'),
      ).toBe('http://test.host/users/auth/cas3?redirect_fragment=L65');

      expect(
        $('.omniauth-container #oauth-login-auth0')
          .parent('form')
          .attr('action'),
      ).toBe('http://test.host/users/auth/auth0?redirect_fragment=L65');
    });
    ...
    it('when "remember-me" is present', () => {
      ...
      expect(
        $('#oauth-login-cas3')
          .parent('form')
          .attr('action'),
      ).toBe('http://test.host/users/auth/cas3?remember_me=1&redirect_fragment=L65');

      expect(
        $('#oauth-login-auth0')
          .parent('form')
          .attr('action'),
      ).toBe('http://test.host/users/auth/auth0?remember_me=1&redirect_fragment=L65');
    });
  });
});
