...
RSpec.describe DesignManagement::CopyDesignCollectionService do
  ...
  context 'when user has permission to read the design collection' do
    ...
    context 'when the user also has permission to admin the target issue' do
      ...
      context 'when design collection has designs' do
        let_it_be(:designs) { create_list(:design, 3, :with_lfs_file, :with_relative_position, issue: issue, project: project) }
        # before_all do
        #   # Create 3 designs
        #   designs = create_list(:design, 3, :with_lfs_file, :with_relative_position, issue: issue, project: project)
        #   # Delete one of the designs in a second version
        #   # create(:design_version, deleted_designs: [designs.first], issue: issue)
        #   DesignManagement::DeleteDesignsService.new(project, user issue: issue, designs: [designs.first]).execute
        # end
        ...
        it 'copies the designs correctly', :aggregate_failures do
          expect { subject }.to change { target_issue.designs.count }.from(0).to(3)

          old_designs = issue.designs.ordered(project)
          new_designs = target_issue.designs.ordered(target_issue.project)
        ...
        it 'copies the design versions correctly', :aggregate_failures do
          expect { subject }.to change { target_issue.design_versions.count }.from(0).to(3)
        ...
        it 'copies the design actions correctly', :aggregate_failures do
          ...
          new_actions.zip(old_actions).each do |new_action, old_action|
            ...
            new_design_filenames = new_action.version.designs.ordered(target_issue.project).pluck(:filename)
            old_design_filenames = old_action.version.designs.ordered(project).pluck(:filename)
        ...
        it 'copies design notes correctly', :aggregate_failures do
        ...
        it 'links the LfsObjects' do
          expect { subject }.to change { target_issue.project.lfs_objects.count }.from(0).to(3)
        end
        ...
        it 'copies the Git repository data' do
          ...
          expect { subject }.to change {
            target_repository.commits('master', limit: 5).size
          }.from(0).to(3)
        end
        ...
        it 'sets a commit description' do
          ...
          expect(target_repository.commit.message).to eq(
        ...
        describe 'rollback' do
          before do
            ...
            expect_next_instance_of(described_class) do |service|
              expect(service).to receive(:copy_commits!).and_wrap_original do |method, *args|
                method.call(*args)

                expect(target_repository).not_to be_empty
          ...
          it 'rollsback all PostgreSQL data created', :aggregate_failures do
            ...
            # expect(target_issue.design_collection).to be_empty
            # expect(target_issue.designs).to be_empty
            expect(target_issue.design_versions).to be_empty
          end

          it 'reverts all Git data created', :aggregate_failures do
            ...
            design_2 = create(:design, :with_file, issue: issue_2, project: target_issue.project)

            subject

            commits = target_repository.commits('master', limit: 10)
            commit_titles = commits.map(&:title)

            # Test for 7 commits in the repository. These are (in the order they will be fetched):
            #  - 3 revert commits (one per design)
            #  - 3 commits made during the copy (one per design)
            #  - 1 commit we made at the start of this test
            expect(commits.size).to eq(7)
            expect(commit_titles[0..2]).to all(match(/^Revert/))
            expect(commit_titles[3..5]).to all(match(/^Copy commit/))
            expect(commit_titles.last).to eq("create for #{design_2.filename}")
          end
        end
      end
    end
