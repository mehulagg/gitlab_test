    ...
    context 'when lfs objects size is larger than the batch size' do
      ...
      context 'when request fails with PayloadTooLarge error' do
        ...
        context 'when the smaller batch eventually works' do
          before do
            stub_const("#{described_class.name}::REQUEST_BATCH_SIZE", 10)

            data = new_oids.to_a

            # with the batch size of 10
          ...
          it 'retreives them eventually and logs exceptions' do
            expect(Gitlab::ErrorTracking).to receive(:track_exception).with(
              an_instance_of(error_class), project_id: project.id, batch_size: 10, oids_count: 5
            )
            expect(Gitlab::ErrorTracking).to receive(:track_exception).with(
        ...
        context 'when batch size cannot be any smaller' do
          before do
            ...
            # with the batch size of 1
            stub_entity_too_large_error_request([data[0]])
          end
          ...
          it 'raises an error and logs exceptions' do
            ...
            expect(Gitlab::ErrorTracking).to receive(:track_exception).with(
              an_instance_of(error_class), project_id: project.id, batch_size: 1, oids_count: 5
            )
