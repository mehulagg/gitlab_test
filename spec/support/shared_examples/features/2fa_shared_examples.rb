...
RSpec.shared_examples 'hardware device for 2fa' do |device_type|
  ...
  describe "registration" do
    ...
    describe 'when 2FA via OTP is disabled' do
      ...
      it 'does not allow registering a new device' do
        ...
        expect(page).to have_button("Set up new #{device_type} device", disabled: true)
      end
    end
  ...
  describe 'fallback code authentication' do
    ...
    def assert_fallback_ui(page, device_type)
      ...
      expect(page).not_to have_css("#js-authenticate-#{device_type.downcase}")
    end
    ...
    describe 'when no device is registered' do
      ...
      it 'shows the fallback otp code UI' do
        assert_fallback_ui(page, device_type)
      end
    end
