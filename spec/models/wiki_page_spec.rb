...
RSpec.describe WikiPage do
  ...
  describe '.group_by_directory' do
    ...
    context 'when there are pages' do
      # Create a new container with an empty wiki for these tests
      let_it_be(:container) { create(:project, :wiki_repo) }

      before_all do
        create_wiki_page(title: 'dir_1/dir_1_1/page_3', content: 'content')
        create_wiki_page(title: 'page_1', content: 'content')
        create_wiki_page(title: 'dir_1/page_2', content: 'content')
        create_wiki_page(title: 'dir_2', content: 'page with dir name')
        create_wiki_page(title: 'dir_2/page_5', content: 'content')
        create_wiki_page(title: 'page_6', content: 'content')
        create_wiki_page(title: 'dir_2/page_4', content: 'content')
      end

      let_it_be(:page_1) { wiki.find_page('page_1') }
      let_it_be(:page_6) { wiki.find_page('page_6') }
      let_it_be(:page_dir_2) { wiki.find_page('dir_2') }

      let_it_be(:dir_1) do
        WikiDirectory.new('dir_1', [wiki.find_page('dir_1/page_2')])
      end

      let_it_be(:dir_1_1) do
