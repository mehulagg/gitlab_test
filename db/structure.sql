...
CREATE INDEX design_mentions_temp_index ON public.notes USING btree (id) WHERE ((note ~~ '%@%'::text) AND ((noteable_type)::text = 'DesignManagement::Design'::text));
...
CREATE INDEX tmp_idx_on_promoted_notes ON public.notes USING btree (note) WHERE (((noteable_type)::text = 'Issue'::text) AND (system IS TRUE) AND (note ~~ 'promoted to epic%'::text));
