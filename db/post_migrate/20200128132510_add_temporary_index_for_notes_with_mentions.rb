...
class AddTemporaryIndexForNotesWithMentions < ActiveRecord::Migration[5.2]
  ...
  SNIPPET_MENTIONS_INDEX_NAME = 'snippet_mentions_temp_index'
  ...
  def up
    ...
    remove_concurrent_index(:notes, :id, name: SNIPPET_MENTIONS_INDEX_NAME)
  end
  ...
  def down
    ...
    add_concurrent_index(:notes, :id, where: "#{INDEX_CONDITION} AND noteable_type='Snippet'", name: SNIPPET_MENTIONS_INDEX_NAME)
  end
end
