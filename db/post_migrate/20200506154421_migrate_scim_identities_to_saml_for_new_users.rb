...
class MigrateScimIdentitiesToSamlForNewUsers < ActiveRecord::Migration[6.0]
  ...
  def up
    ScimIdentity.each_batch(of: 100) do |relation|
      ...
      ActiveRecord::Base.connection.execute("insert into identities (extern_uid, provider, user_id, created_at, updated_at, saml_provider_id) #{identity_records.to_sql} on conflict do nothing")
    end
  end
