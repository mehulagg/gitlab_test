...
- To see a large `.gitlab-ci.yml` file used in an enterprise, see the [`.gitlab-ci.yml` file for `gitlab-foss`](https://gitlab.com/gitlab-org/gitlab-foss/blob/master/.gitlab-ci.yml).
...
job:
  only:
    ...
    - /^release/.*$/@gitlab-org/gitlab-foss
```

The above example will run `job` for all branches on `gitlab-org/gitlab-foss`,
