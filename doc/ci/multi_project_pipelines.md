...
NOTE: **Note:**
If using variables or [`rules`](yaml/README.md#rulesif) to control job behavior,
the value of the [`$CI_PIPELINE_SOURCE` predefined variable](variables/predefined_variables.md)
is `pipeline` for multi-project pipeline triggered through the API with `CI_JOB_TOKEN`.
If using [`only/except`](yaml/README.md#onlyexcept-basic) to control job behavior,
use the `pipelines` keyword.
...
If using variables or [`rules`](yaml/README.md#rulesif) to control job behavior,
the value of the [`$CI_PIPELINE_SOURCE` predefined variable](variables/predefined_variables.md)
is `pipeline` for multi-project pipelines triggered with a bridge job (using the
[`trigger:`](yaml/README.md#trigger) keyword). If using [`only/except`](yaml/README.md#onlyexcept-basic)
to control job behavior, use the `pipelines` keyword.
