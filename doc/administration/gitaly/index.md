...
`401 Unauthorized` message, then it's likely Gitaly is failing to authenticate
with the other nodes due to having a [wrong secrets file](#3-gitaly-server-configuration).
...
- When any user adds or modifies a file from the repository using the GitLab
  UI, it immediatley fails with the same `401 Unauthorized` message.
- Creating a new project and [initializing it with a README](../../gitlab-basics/create-project.md#blank-projects)
  successfully creates the project but doesn't create the README.
- The `api_json.log` and `nginx_access.log` each have **401** errors when
  hitting the `/api/v4/internal/allowed` endpoint:
