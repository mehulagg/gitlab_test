...
1. Configure PostgreSQL to listen on network interfaces:
    ...
    ##
    postgresql['listen_address'] = '198.51.100.111'
...
1. Optional: If you want to add another **secondary** node, the relevant setting would look like:
    ...
    ```ruby
    postgresql['md5_auth_cidr_addresses'] = ['198.51.100.111/32', '198.51.100.222/32','198.51.100.3/32']
    ```
...
1. [Check TCP connectivity][rake-maintenance] to the **primary** node's PostgreSQL server:
    ...
    ```sh
    gitlab-rake gitlab:tcp_check[198.51.100.111,5432]
    ```
...
1. Test that the `gitlab-psql` user can connect to the **primary** node's database:
    ...
    ```sh
    sudo -u gitlab-psql /opt/gitlab/embedded/bin/psql --list -U gitlab_replicator -d "dbname=gitlabhq_production sslmode=verify-ca" -W -h 198.51.100.111
    ```
...
1. Configure PostgreSQL to enable FDW support:
    ...
    ## - replace '198.51.100.222' with the public or VPC address of your Geo secondary node
    ##
    postgresql['listen_address'] = '198.51.100.222'
    postgresql['md5_auth_cidr_addresses'] = ['198.51.100.222/32']
...
1. Execute the command below to start a backup/restore and begin the replication
    ...
    ```sh
    gitlab-ctl replicate-geo-database --slot-name=secondary_example --host=198.51.100.111
    ```
