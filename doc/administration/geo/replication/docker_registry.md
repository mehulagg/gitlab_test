...
1. Edit `/etc/gitlab/gitlab.rb`:
   ...
   ```
   NOTE: **Note:**
   Replace `<replace_with_a_secret_token>` with a case sensitive alphanumeric string that starts with a letter. i.e. you can generate one with `< /dev/urandom tr -dc _A-Z-a-z-0-9 | head -c 32 | sed "s/^[0-9]*//"; echo`
