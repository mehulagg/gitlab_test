...
- The IP of the **primary** server for our examples is `198.51.100.111`, whereas the
  **secondary** node's IP is `198.51.100.222`. Note that the **primary** and **secondary** servers
...
1. Set the access control on the **primary** node to allow TCP connections using the
   ...
   `/etc/postgresql/9.x/main/pg_hba.conf`):
    ...
    ```sh
    host    all             all                      198.51.100.111/32      md5
    host    replication     gitlab_replicator        198.51.100.222/32      md5
    ```

    Where `198.51.100.111` is the public IP address of the **primary** server, and `198.51.100.222`
    ...
    host    all             all                      198.51.100.111/32      md5
    host    replication     gitlab_replicator        198.51.100.222/32      md5
    host    replication     gitlab_replicator        198.51.100.333/32  md5
    ```
