...
the `commit_committer_check`, `commit_author_check`, and `reject_unsigned_commits` parameters:
...
| `commit_author_check` **(PREMIUM)**           | boolean        | no       | Users can only push commits to this repository that were authored with one of their own verified emails. |
