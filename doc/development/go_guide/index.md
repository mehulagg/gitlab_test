...
- Use `goimports -local gitlab.com` before committing.
  ...
  By using the `-local gitlab.com` option, `goimports` will group locally referenced
  packages separately from external ones.

  Most text editors will allow you to set up a `before-save` command to automatically
  execute `goimports` so that it's applied to every file when saving.
