...
- It's ok to look for DOM elements but don't abuse it since it makes the tests
  more brittle
- Avoid using `data-qa-selector` for locating elements as they are meant for End to End (e2e) tests. There is a risk that those selectors would be changed without in an e2e test MR where unit tests are not run resulting in a broken master.   
