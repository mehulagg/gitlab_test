...
data to be split into smaller physical tables that still act as a single
large table. If the data access patterns support the partitioning layout,
the database can cheaply eliminate much of the data from the search
space, while still providing full SQL capabilities.

## Determining When to Use Partitioning

While partitioning can be very useful when properly applied, the first
step is identifying if the data and workload of a table naturally fit a
...
use to split the data across the partitions. The scheme used by GitLab is
date-range partitioning, where each partition in the table contains data
for a single month. In this case, the partitioning key would need to be
a timestamp or date column. In order for this type of partitioning to work
well, most queries would need to access data within a certain date range.

For a more concrete example, the `audit_events` table can be used, which
was the first table to be partitioned in the application database. This
