...
Currently, the technical writing team strongly encourages included documentation in
...
of their definition of done. When using this workflow, a team's engineers must include
their docs in the **same** MR as their feature code, at all times.

#### Downsides of separate docs merge requests

A workflow that has documentation separated into it's own MR has many downsides:
...
- The docs and code will be merged at different times, which has caused many problems in the past:
  - If the documentation merges *before* the feature:
    - GitLab.com users may try to use the feature before it's released, driving support tickets.
    - If the feature is delayed, the documentation may not be pulled/reverted in time
  ...
  - If the documentation merges *after* the feature:
    - The feature may be included in the self-hosted package, but without any documentation
      if the docs MR misses the cutoff.
    - UI for a feature may show up on GitLab.com before any documentation exists for it.
...
- Having two separate MRs means two separate people will be responsible for merging one
  feature, which is not workable with an asynchronous work style. The feature may
...
- In a "split" workflow, engineers may only create the documentation MR once the feature MR
...
- The Technical Writer will have more time to gain a deep understanding of the feature,
  ...
  the feature when possible. They will also be able to offer advice for improving
  the text in UI elements, offer alternative use cases, or explain how well (or not)
  the feature worked in testing.
...
- Increasing visibility of the documentation also has the side effect of improving
  *other* engineer's documentation. By being exposed to more docs, through reviewing
  other engineer's MR, their own documentation skills will improve.
...
In order to have Docs-with-Code as a mandatory workflow, some changes may need to
...
- Reviewers and Maintainers must also review the docs during code reviews, to make
  ...
  are correct. They do not need to worry about style, grammar, etc.
- The technical writer must be assigned the MR directly and not only pinged. Thanks
  ...
  this can be done at any time, but must be before the code maintainer review. It is
...
- When the docs are ready, the technical writer will click **Approve** and usually
  ...
  update.
- Maintainers are allowed to merge features with the docs "as-is" to ensure a feature
  meets a deadline such as a release cutoff, even if the technical writer has not given
  final approval yet. The **docs must not be blockers**. Therefore it is key to get
  the docs included and assigned to the technical writers early. If the feature is
  merged before final docs approval, the maintainer must create a [post-merge follow-up issue](#post-merge-reviews),
